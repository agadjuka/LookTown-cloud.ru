...
5. Если клиент выбирает мастера — извлеки `master_id` (int) или `master_name` (str). ("мастер" и "топ мастер" не являются ID или Именем, это категории услуг, используй их для выбора услуги)
...
- Клиент не видит ID, он пишет название текстом, или номер из списка. Ты должен найти ID из ToolResult. Клиент не будет писать точное название, ты должен понять что он имеет ввиду."""
