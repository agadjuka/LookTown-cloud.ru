...
Исключение (не делай этого при стандартном процессе записи): Если клиент хочет узнать подробности об услуге (спрашивает детали какой либо услуги), мастере (инетересуется мастером который делает эту услугу), мастерах которые делают услугу тогда: Добавь в JSON поле `"service_details_needed": true`
