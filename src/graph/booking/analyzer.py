...
7. INTENT (user_intent):
   - "booking_action": Клиент дает данные, соглашается ("Да", "Ок") или выбирает слот.
   - "question": Клиент задает вопрос (про мастеров, цены, адрес, больно ли это).
   - "change_input": Клиент явно пишет "нет, давай другое", "поменяй время".
   
   ВАЖНО ПРО СМЕШАННЫЕ СООБЩЕНИЯ:
   Если клиент дает данные И задает вопрос (например: "Запиши к Анне, а какой у нее опыт?"), ты должен:
   1. ИЗВЛЕЧЬ данные (master_name="Анна").
   2. УСТАНОВИТЬ user_intent="question" (чтобы мы могли ответить на вопрос).
