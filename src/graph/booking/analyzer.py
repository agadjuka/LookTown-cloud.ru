...
- СМЕНА УСЛУГИ ИЛИ МАСТЕРА: Если клиент хотел записаться на одну услугу (service_id уже заполнен) или к конкретному мастеру (master_id уже заполнен) а потом решил поменять услугу или мастера, ты ОБЯЗАН сбросить `service_id` и `slot_time` в null. Но если меняется услуга на ту, ID которой тебе известно - отправляй правильный ID.
