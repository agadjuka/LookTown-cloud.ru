...
- about_salon: Client asks about addresses, contact information, phone numbers, social networks, of salon ("расскажите про салон", "где вы находитесь", "как с вами связаться", "ваш адрес", "телефон салона"). NOT ABOUT SERVICES
...
"""
    ...
    def run(self, message: str, history: Optional[List[Dict[str, Any]]] = None, chat_id: Optional[str] = None) -> Dict[str, Any]:
        """
        Переопределение метода run() для stage_detector
        Напоминание добавляется в orchestrator перед последним сообщением пользователя
        """
        # Вызываем родительский метод - напоминание будет добавлено в orchestrator
        return super().run(message, history, chat_id=chat_id)
